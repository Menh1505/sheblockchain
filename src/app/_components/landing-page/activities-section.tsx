    ...
    <section className="mx-auto scroll-mt-32 px-6 py-8 sm:px-8 lg:px-16">
      ...
      <div className="mt-8 flex items-center justify-center gap-4">
        <button
          ...
          className="inline-flex h-14 w-14 items-center justify-center rounded-full border-2 border-gray-700 bg-gray-800 text-[#00ff88] shadow-[4px_4px_0_#00ff88] transition hover:-translate-y-0.5"
        >
          <ArrowLeftIcon />
        </button>
        ...
        <div className="flex items-center gap-2">
          {slides.map((slide, index) => (
            <button
              ...
              className={`h-3 rounded-full transition-all ${activeIndex === index ? "w-10 bg-[#00ff88]" : "w-3 bg-gray-600"
                }`}
            />
          ))}
        </div>
